...
module "budgets" {
  ...
  budgets = {
    lookup(each.value.budget, "name") = {
      ...
      # update_rules = {
      #   lookup(each.value.budget, "name", "Rule for ${each.value.name}") = {
      #     disable_default_iam_recipients = try(
      #       each.value.budget.notifications.notify_default_recipients, false
      #     )
      #     monitoring_notification_channels = (try(each.value.budget.notifications.email_recipients, null) == null
      #       ? null
      #     : each.value.budget.notifications.email_recipients)
      #     pubsub_topic = try(
      #       each.value.budget.notifications.pubsub_topic, null
      #     )
      #   }
      # }
